...
export class SleepPage {
  ...
  private async loadSleepYoga() {
    ...
    try {
      ...
      this.sleepYoga = this.filterSleepYoga(cachedNormalized);
  ...
  private filterSleepYoga(list: any[]): any[] {
    const selectedLanguage = this.isPortuguese ? 'PT' : 'EN';
    const word = this.normalize('sleep');
