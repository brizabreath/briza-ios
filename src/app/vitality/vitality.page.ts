...
@Component({
  ...
  templateUrl: './vitality.page.html',
  styleUrls: ['./vitality.page.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, IonicModule, CoreModule, RouterModule],
})
export class VitalityPage {
  ...
  private async loadVitalityYoga() {
    ...
    try {
      ...
      this.vitalityYoga = this.filterVitalityYoga(cachedNormalized);
  ...
  private filterVitalityYoga(list: any[]): any[] {
    const selectedLanguage = this.isPortuguese ? 'PT' : 'EN';
    const word = this.normalize('vitality');
