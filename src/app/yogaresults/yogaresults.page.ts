...
@Component({
  selector: 'app-yogaresults',
  templateUrl: './yogaresults.page.html',
  styleUrls: ['./yogaresults.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class YogaresultsPage implements AfterViewInit {
  constructor(private navCtrl: NavController, private globalService: GlobalService) {}

  @ViewChild('yogaChartCanvas') yogaChartCanvas!: ElementRef<HTMLCanvasElement>;
  yogaChart: any;
  ...
  loadDataFromLocalStorage(): void {
    const exerciseKeys = ['BBResults', 'YBResults', 'BREResults', 'BRWResults', 'CTResults', 'APResults', 'UBResults', 'BOXResults', 'CBResults', 'RBResults', 'NBResults', 'CUSTResults', 'AHATResults', 'HATResults', 'HATCResults', 'KBResults', 'WHResults', 'LungsResults', 'YogaResults'];
  ...
  initializeChart(): void {
    const context = this.yogaChartCanvas.nativeElement.getContext('2d');
    if (context) {
      this.yogaChart = new Chart(context, {
        type: 'bar',
        data: {
  ...
  updateChart(startDate: Date, endDate: Date): void {
    ...
    this.yogaChart.data.labels = this.selectedDataDates;
    this.yogaChart.data.datasets[0].data = this.chartData;
    this.yogaChart.options.scales.y.max = this.maxYValue;
    this.yogaChart.options.scales.y.ticks.stepSize = this.calculateStepSize(); // Update step size dynamically
    this.yogaChart.options.plugins.title.text = this.getChartTitle(); // Update title with current year
    this.yogaChart.update();
  }
  ...
  setupScrolling(): void {
    let lastScrollX = 0;
    this.yogaChartCanvas.nativeElement.addEventListener('touchstart', (event: TouchEvent) => lastScrollX = event.touches[0].clientX);
    this.yogaChartCanvas.nativeElement.addEventListener('touchmove', (event: TouchEvent) => {
      event.preventDefault();
      const deltaX = event.touches[0].clientX - lastScrollX;
      this.handleScroll(-deltaX);
      lastScrollX = event.touches[0].clientX;
    });
    this.yogaChartCanvas.nativeElement.addEventListener('wheel', (event: WheelEvent) => this.handleScroll(-event.deltaX));
  }
