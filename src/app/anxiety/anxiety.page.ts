...
export class AnxietyPage {
  ...
  private async loadAnxietyYoga() {
    try {
      ...
      this.anxietyYoga = this.filterAnxietyYoga(cachedNormalized);
  ...
  private filterAnxietyYoga(list: any[]): any[] {
    const selectedLanguage = this.isPortuguese ? 'PT' : 'EN';
    const word = this.normalize('anxiety');
