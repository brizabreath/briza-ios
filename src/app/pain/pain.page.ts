...
export class PainPage {
  ...
  private async loadPainYoga() {
    try {
      ...
      this.painYoga = this.filterPainYoga(cachedNormalized);
      ...
      this.painYoga = this.filterPainYoga(serverVideos);
    } finally {
    }
  }

  private filterPainYoga(list: any[]): any[] {
    const selectedLanguage = this.isPortuguese ? 'PT' : 'EN';
    const word = this.normalize('pain');
