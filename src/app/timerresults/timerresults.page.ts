...
@Component({
  selector: 'app-timerresults',
  templateUrl: './timerresults.page.html',
  styleUrls: ['./timerresults.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class TIMERresultsPage implements AfterViewInit {
  ...
  initializeChart(): void {
    const context = this.timerChartCanvas.nativeElement.getContext('2d');
    if (context) {
      this.timerChart = new Chart(context, {
        type: 'bar',
        data: {
  ...
  updateChart(startDate: Date, endDate: Date): void {
    ...
    this.timerChart.data.labels = this.selectedDataDates;
    this.timerChart.data.datasets[0].data = this.chartData;
    this.timerChart.options.scales.y.max = this.maxYValue;
    this.timerChart.options.scales.y.ticks.stepSize = this.calculateStepSize(); // Update step size dynamically
    this.timerChart.options.plugins.title.text = this.getChartTitle(); // Update title with current year
  ...
  setupScrolling(): void {
    let lastScrollX = 0;
    this.timerChartCanvas.nativeElement.addEventListener('touchstart', (event: TouchEvent) => lastScrollX = event.touches[0].clientX);
    this.timerChartCanvas.nativeElement.addEventListener('touchmove', (event: TouchEvent) => {
      event.preventDefault();
      const deltaX = event.touches[0].clientX - lastScrollX;
      this.handleScroll(-deltaX);
      lastScrollX = event.touches[0].clientX;
    });
    this.timerChartCanvas.nativeElement.addEventListener('wheel', (event: WheelEvent) => this.handleScroll(-event.deltaX));
  }
